...
export default function CalendarGrid({ 
  ...
  const BASE_ROW_HEIGHT = 110 // Altura predeterminada que se ve bien
  const EVENT_HEIGHT = 23 // Altura de cada evento
  const BUTTON_SPACE = 25 // Espacio para botones "Ver más/menos"
  ...
  // 1. AÑADIMOS DE NUEVO LA FUNCIÓN handleEventClick
  const handleEventClick = (event) => {
    // Primero, cerramos el tooltip
    setHoveredEvent(null)
    // Luego, llamamos al callback del padre (que abre el modal)
    if (onEventClick) {
      onEventClick(event)
    }
  }
                ...
                {eventLayouts.map(({ event, track, startCol, span, isStart }, idx) => {
                  ...
                  return (
                    <div
                      key={`${event.id}-${idx}`}
                      // ************************************************
                      // *** 2. ACTUALIZAMOS EL onClick DEL EVENTO ***
                      // ************************************************
                      onClick={(e) => { 
                        e.stopPropagation()
